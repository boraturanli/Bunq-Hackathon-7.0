...
class BunqLib(object):
    ...
    def setup_context(self, reset_config_if_needed=True):
        ...
        except (ForbiddenException, BadRequestException) as e:
            if reset_config_if_needed and self.env == ApiEnvironmentType.SANDBOX:
    ...
    def make_payment(self, amount_string: str, description: str, recipient: str, pointer_type: str = "EMAIL"):
        Payment.create(
            amount=Amount(amount_string, self._CURRENCY_EUR),
            counterparty_alias=Pointer(pointer_type, recipient),
            description=description
        )

    def make_request(self, amount_string: str, description: str, recipient: str, pointer_type: str = "EMAIL"):
